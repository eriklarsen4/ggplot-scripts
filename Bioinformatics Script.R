...
e13_PATHWAYS = add_column(e13_PATHWAYS, Modified_Combined_Score = (e13_PATHWAYS$EnrichrZscore)*(log10(e13_PATHWAYS$Panther_2016.Adjusted.P.value)), .before = 6)
    ...
    ## Find genes within a Process or Pathway of Interest that are DE (down) in mutants
...
Neuron_Diff_Overlap = c(which(DESeq2_e133$GeneID %in% e13_GO_GENES[6,1:e13_GOs$Overlap[6]]), which(DESeq2_e133$GeneID == "Neurog1"), which(DESeq2_e133$GeneID == "Neurog2"), which(DESeq2_e133$GeneID == "Tlx3"))
      ## Concatenate another list of the indeces from the overlapping genes are down in mutants
...
#e13_Genes_and_Terms = data.frame(1:nrow(DESeq2_e133), 2)
#colnames(e13_Genes_and_Terms) = c("GeneID", "Terms")
#e13_Genes_and_Terms$GeneID = ""
#e13_Genes_and_Terms$GeneID = as.character(DESeq2_e133$GeneID)
#e13_Genes_and_Terms$Terms = ""
...
#which(DESeq2_e133$GeneID %in% c(e13_GO_GENES[6,1:e13_GOs$Overlap[6]]))
#e13_GO_GENES[6,1:26]
#which(DESeq2_e133$GeneID == "Fzd10")
#which(DESeq2_e133$GeneID == "Neurod1")
#which(DESeq2_e133$GeneID == "Mapk9")
#which(DESeq2_e133$GeneID == "Neurod4")
#which(DESeq2_e133$GeneID == "Wnt4")
#which(DESeq2_e133$GeneID == "Fzd2")
#which(DESeq2_e133$GeneID == "Wnt3a")
#which(DESeq2_e133$GeneID == "Fzd8")
#which(DESeq2_e133$GeneID == "Pou4f2")
#which(DESeq2_e133$GeneID == "Mapk10")
...
DESeq2_e133$labs2 = ""
DESeq2_e133$labs2[c(34,97,278,678,790,803,811,829,1186,1431)] = DESeq2_e133$GeneID[c(34,97,278,678,790,803,811,829,1186,1431)]
...
vole13 = ggplot(DESeq2_e133) +
  ...
  scale_color_manual(values = c("goldenrod4", "navy", "gray87"))
...
MAe13 = ggplot(DESeq2_e133) +
  ...
  labs(x = "ln(Mean of Normalized Counts)", y = "log2(FC)", title = "e13 Tmem184b GT DRG Gene Exp. Changes", col = "Gene Data Type") +
  geom_hline(yintercept = 0, type = "solid", color = "black") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5)) + 
  scale_color_manual(values = c("goldenrod4", "navy", "gray67"))
...
##### Create a Dendrogram Heatmap using Normalized Counts #####
  ## Import the e13 normalized counts file. These are expression estimates for each gene, where each gene's value is scaled by its sample's effect size
...
e13_Norm_scaled = e13_Normalized_Counts
e13_Norm_scaled[,2:7] = scale(e13_Norm_scaled[,2:7])
e13_Norm_scaled_MAT = as.matrix(e13_Norm_scaled[, -1])
rownames(e13_Norm_scaled_MAT) = e13_Norm_scaled$GeneID
#dend = as.dendrogram(hclust(d = dist(x = e13_Norm_scaled_MAT)))
...
##### Volcano Plot of Adult GT DRG #####
  ...
  ## Create a column in the DESeq2 dataframe that scales the Adjusted P-value by log-base 10
DESeq2_Adults3$log10Pval = -log10(DESeq2_Adults3$AdjP)

  ## Create the data points of interest for functional display.. ggplot struggles to plot subsets of dataframes, so creating a variable that directly labels all the points accordingly helps.
  ## Create a new column (genes of interest); Using "GeneID" is irrelevant; the values will be replaced in the next step
DESeq2_Adults3$g.o.i. = DESeq2_Adults3$GeneID

  ## Fill the points with appropriately indexed data;
  ## "Differentially Expressed Genes" are defined as being "genes of interest" with Adjusted P-Values of < 0.05
DESeq2_Adults3$g.o.i.[which(DESeq2_Adults3$AdjP <= 0.05)]= "DEGs"
...
DESeq2_Adults3$g.o.i.[which(DESeq2_Adults3$AdjP >= 0.05)]= "Non-DEGs"

  ## Subset of genes with well-known functions and effects on DRG pain/itch behavior, according to literature
DESeq2_Adults3$g.o.i.[c(2,5,8,9,13,14,21,31,34,46,54,74,80,101,118,128,145,208,250,268,352,353,354,356)] = "Itch-related DEGs"
...
#ix_label1 = c(DESeq2_Adults3$GeneID[DESeq2_Adults3$log10Pval > 28])
  ...
  ## Fill the appropriate rows of the "labs" column with the corresponding gene names for labeling those points
DESeq2_Adults3$labs[c(c(1,2,5,8,9,13,21,31,54,118,352,356))] = DESeq2_Adults3$GeneID[c(c(1,2,5,8,9,13,21,31,54,118,352,356))]
...
voladult = ggplot(DESeq2_Adults3) +
  ...
  geom_point(data = subset(DESeq2_Adults3, `g.o.i.` == "Itch-related DEGs"),
             aes(x = `log2.FC.`, y = `log10Pval`, color = `g.o.i.`), alpha = 0.8) +
  coord_cartesian(xlim = c(-2,2), ylim = c(0,20)) +
  labs(x = "log2(FC)", y = "-log10(Adj.P-val)", title = "Adult Tmem184b GT DRG Gene Exp. Changes", col = "Gene Data Type") +
  ...
  scale_color_manual(values = c("goldenrod4", "navy", "gray48"))

  ## geom_text_repel labels the selected points in the color commanded
  ## Re-save the variable with the added functionality; for unlabeled plot
voladult = voladult  + geom_text_repel(data = DESeq2_Adults3, x = DESeq2_Adults3$log2.FC., y = DESeq2_Adults3$log10Pval, color = "black", aes(label = DESeq2_Adults3$labs)) 
...
Adult_MA = ggplot(DESeq2_Adults3) +
  ...
  labs(x = "log(Mean of Normalized Counts)", y = "log2(FC) mRNA Rel. to WT", title = "Adult DRG Tmem184b GT mRNA Exp.", col = "Gene Data Type") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5)) +
...
##### Adult Panther Pathways Heatmap with labels using ggplot #####
  ## Import the relevant data
...
##### Adult Panther Pathways Heatmap without labels using ggplot #####
...
##### Adult Global Expression Heatmap #####
  ## Import the e13 normalized counts file. These are expression estimates for each gene, where each gene's value is normalized to its sample's effect size
...
##
  ## Create a duplicate of the Adult DRG GT Normalized Counts file from Galaxy/DESeq2
Adult_Norm_scaled = Adult_Normalized_Counts
  ## Create another duplicate of the Adult DRG GT Normalized Counts file from Galaxy/DESeq2
Adult_Norm_scaled_center = Adult_Normalized_Counts
  ## Scale the data
Adult_Norm_scaled[,2:9] = scale(x = Adult_Norm_scaled[,2:9], center = FALSE, scale = TRUE)
  ## Scale AND center the data
Adult_Norm_scaled_center[,2:9] = scale(x = Adult_Norm_scaled[,2:9], center = TRUE, scale = TRUE)
  ## Make the scaled data a matrix and remove the GeneID
Adult_Norm_scaled_MAT = as.matrix(Adult_Norm_scaled[, -1])
  ## Make the row indeces the GeneIDs
rownames(Adult_Norm_scaled_MAT) = Adult_Norm_scaled$GeneID
  ## Make the scaled AND CENTERED data a matrix and remove the GeneIDs
Adult_Norm_scaled_center_MAT = as.matrix(Adult_Norm_scaled_center[, -1])
  ## Make the row indeces the GeneIDs
rownames(Adult_Norm_scaled_center_MAT) = Adult_Norm_scaled_center$GeneID

  ## Subset only the DEGs
Adult_Norm_scaled_DEGs = Adult_Norm_scaled_center[which(Adult_Norm_scaled_center$GeneID %in% DESeq2_Adults3$GeneID[which(DESeq2_Adults3$AdjP < 0.05)]),]

adult.dendro = as.dendrogram(hclust(d = dist(x = Adult_Norm_scaled_center_MAT)))

ggd = ggdendrogram(data = adult.dendro, rotate = TRUE)
print(ggd)
...
Adult_GOs = Adult_GOs[order(Adult_GOs$GO_Biological_Process_2018.Adjusted.P.value, decreasing = FALSE), ]
  ## Re-set the index to make sure indeces are correct
row.names(Adult_GOs) = NULL

  ## Subset the dataframe by the results obtained in the Enrichr search, identifying "neuron differentiation" as a significantly affected biological process:
  ## For all genes in the term
#RNASeqRepResultsNeurDiff = e13_Normalized_Counts[e13_Normalized_Counts$GeneID %in% c(neuron_differentiation),]
  ## For only the overlapping (DE) genes
#RNASeqRepResultsNeurDiff = e13_Normalized_Counts[e13_Normalized_Counts$GeneID %in% c(e13_GO_GENES[6,1:e13_GOs$Overlap[6]]),]
  ## For only detectable genes
...
RNASeqRepResultsAdultAllZ = RNASeqRepResultsAdultAllZ[!row_has_na,]
  ...
  ## Find limits
#max(RNASeqRepResultsAdultAllZ[,2:9])
#min(RNASeqRepResultsAdultAllZ[,2:9])

  ## Transform the profile to include only DEGs
RNASeqRepResultsAdultAllZA = RNASeqRepResultsAdultAllZ %>%
 filter(GeneID %in% DESeq2_Adults3$GeneID[c(1:405)])
...
## Find where the "Itch-related DEGs" are in the clustered matrix for subsetting the heatmap
which(Euclid_dist_ord_Genes == "Il31ra") # 360
which(Euclid_dist_ord_Genes == "Cysltr2") # 359
which(Euclid_dist_ord_Genes == "Npy2r") # 396
which(Euclid_dist_ord_Genes == "Sst") # 341
which(Euclid_dist_ord_Genes == "Htr1a") # 143
which(Euclid_dist_ord_Genes == "P2rx3") # 385
which(Euclid_dist_ord_Genes == "Lpar3") # 395
which(Euclid_dist_ord_Genes == "Lpar5") # 147
which(Euclid_dist_ord_Genes == "Scn11a") # 164
which(Euclid_dist_ord_Genes == "Scn10a") # 172
which(Euclid_dist_ord_Genes == "Mrgprd") # 163
which(Euclid_dist_ord_Genes == "Trpc6") # 180
which(Euclid_dist_ord_Genes == "Trpc3") # 94
which(Euclid_dist_ord_Genes == "F2rl2") # 270
which(Euclid_dist_ord_Genes == "Htr1f") # 137
which(Euclid_dist_ord_Genes == "Osmr") # 351
which(Euclid_dist_ord_Genes == "Fxyd2") # 189
which(Euclid_dist_ord_Genes == "Htr4") # 181
which(Euclid_dist_ord_Genes == "Mrgprx1") # 130
which(Euclid_dist_ord_Genes == "Ptgdr") # 183
which(Euclid_dist_ord_Genes == "Trpa1") # 208
which(Euclid_dist_ord_Genes == "Trpm6") # 348
which(Euclid_dist_ord_Genes == "Hrh1") # 91
which(Euclid_dist_ord_Genes == "Mrgpra3") # 119

  ## Confirm by indexing; copy and paste to customize the clustered heatmap
Euclid_dist_ord_Genes[c(360,359,396,341,143,385,395,147,164,172,163,180,94,270,137,351,189,181,130,183,208,348,91,119)]
  ## Confirm the location/order of the transformed matrix/dataframe
    ## Should be "Il31ra"
RNASeqRepResultsAdultAllZA[360,]

      ## The "inversion" or "break point" of the entire DEG heatmap is row 90/91, "Hrh1"

  ## Visualize the Z-scored, Euclidean-clustered and ordered Genes TPMs across replicates with "pheatmap"
...
pheatmap(mat = RNASeqRepResultsAdultAllZA[139:149,2:9], color = colorRampPalette(c("navy", "white", "darkgoldenrod4"))((50)), clustsering_distance_rows = "euclidean", angle_col = 0, treeheight_row = 35, treeheight_col = 9, labels_row = Euclid_dist_ord_Genes[139:149])
  ## Visualize the "Itch-related DEGs"
pheatmap(mat = RNASeqRepResultsAdultAllZA[c(360,359,396,341,143,385,395,147,164,172,163,180,94,270,137,351,189,181,130,183,208,348,91,119),2:9], color = colorRampPalette(c("navy", "white", "darkgoldenrod4"))((50)), clustsering_distance_rows = "euclidean", angle_col = 0, treeheight_row = 35, treeheight_col = 9, labels_row = Euclid_dist_ord_Genes[c(360,359,396,341,143,385,395,147,164,172,163,180,94,270,137,351,189,181,130,183,208,348,91,119)])
...
##### In vitro GO Bio Processes and Panther Pathways Heatmaps with labels using ggplot #####
  ## Import the relevant data
...
##### Rescue GO Bio Processes and Panther Pathways Heatmaps with labels using ggplot #####
  ## Import the relevant data
...
Tmem_Direct_Target_Candidates = c(Tmem_Direct_Targets[Tmem_Direct_Target_Candidates_Index])
  ...
  ## Use the new list for GO/Pathway analysis; also create new dataframes by indexing each relevant dataframe by this list
#POSITIVELY_REGULATED_DIV0 = DESeq2_In_vitro_DIV143[Tmem_Direct_Target_Candidates,]
#POSITIVELY_REGULATED_DIV14 = DESeq2_In_vitro_DIV143[Tmem_Direct_Target_Candidates,]
#POSITIVELY_REGULATED_RESCUE = DESeq2_In_vitro_Rescue3[Tmem_Direct_Target_Candidates,]
...
##### Intersection Rescue GO Bio Processes and Panther Pathways Heatmaps with labels using ggplot #####
  ## Import the relevant data
